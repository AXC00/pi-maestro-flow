...
test("real connect --stdio performs initialize, list, and call through the daemon", async (t) => {
  ...
  assert.deepEqual(tools.tools.map((tool) => tool.name), ["workspace", "board", "host", "exec", "job", "file", "teammate", "session", "todo", "monitor", "handoff", "skill", "maestro_cli", "browser", "device", "endpoint", "route"]);
