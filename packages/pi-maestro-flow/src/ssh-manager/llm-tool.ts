...
export const SshToolParams = Type.Object({
  ...
  description: "List unlocked SSH targets, execute a remote command, or use the built-in Gateway. Pass command without action to run a shell command. Pass action without command for listing or Gateway. ensure_gateway may start a non-persistent remote daemon tied to the current local Pi session. targetId selects a provider-owned configured server; omission works only when exactly one #ssh server is attached and errors for none or multiple. Host, authentication, and Gateway command parameters are never accepted. For dynamic Gateway calls, use action=describe tool=<name> to retrieve the authoritative inputSchema before action=call. session.start-pi returns taskId and monitorHandle; pass either value as monitor.handle.",
});
